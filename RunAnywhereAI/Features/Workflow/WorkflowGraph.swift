...
struct WorkflowNode: Identifiable, Equatable {
    ...
    var argumentPorts: [WorkflowToolPort] {
        switch kind {
        case .toolCall, .packNode: settings.toolPorts
        default: []
        }
    }
}
...
extension WorkflowNode {
    ...
    private var triggerSummary: String {
        ...
        case .interval, .unspecified, .UNRECOGNIZED:
            return "Every "
                + WorkflowScheduleFormat.interval(seconds: settings.scheduleIntervalSeconds)
        }
    }
    ...
    private var logicSummary: String {
        ...
        case .condition, .filter:
            ...
            return "\(settings.conditionLeft) \(settings.conditionOperator.symbol) "
                + settings.conditionRight
        case .loopOverItems:
...
///
/// The values that are not design-system spacing are the card's own drawn
/// dimensions; this table is the one place they are decided.
enum WorkflowCanvasMetrics {
    static let cardWidth: CGFloat = 240
    static let headerHeight: CGFloat = 34
    static let detailsHeight: CGFloat = 70
    static let portRowHeight: CGFloat = 24
    static let railTopPadding = Space.xs
    static let railBottomPadding = Space.sm
    ...
    static let snapRadius = Space.xxl
    static let minZoom: CGFloat = 0.25
    static let maxZoom: CGFloat = 2.0
    static let gridStep = Space.sm
    static let gridSpacing = Space.xl
...
extension RAModelCategory {
    var workflowLabel: String {
        ...
        case .voiceActivityDetection: "Voice activity"
        case .speakerDiarization: "Diarization"
        case .semanticSegmentation: "Segmentation"
        case .rerank: "Rerank"
        case .audio: "Audio"

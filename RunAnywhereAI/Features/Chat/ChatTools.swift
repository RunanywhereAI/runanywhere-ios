...
enum ChatTools {
    ...
    static let skill: String = {
        guard let url = Bundle.main.url(forResource: "Skill", withExtension: "md"),
            ...
            return """
                You have two tools: web_research, which searches the live web and                 answers from the pages it reads, and calculate, which evaluates one                 arithmetic expression. Use web_research for anything you cannot know                 from training alone. Answer from tool results, never from memory,                 whenever a tool ran.
                """
        }
        return text
    }()

    static func registerBuiltIns() async {
        ...
        // The commons `web_research` provider. It replaces the per-binding
        // `search_web`, so only one of the two is offered or the model picks

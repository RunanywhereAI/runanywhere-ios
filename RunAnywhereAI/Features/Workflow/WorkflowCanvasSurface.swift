...
struct WorkflowCanvasCamera: Equatable {
    var zoom = CGFloat(1)
    var pan = CGSize(width: Space.xxl * 1.5, height: Space.xxl * 1.5)
    ...
    mutating func magnify(by factor: CGFloat, about viewPoint: CGPoint) {
        let target = min(
            max(zoom * factor, WorkflowCanvasMetrics.minZoom),
            WorkflowCanvasMetrics.maxZoom
        )
...
struct WorkflowCanvasSurface: View {
    ...
    private func drawGrid(in context: inout GraphicsContext, size: CGSize) {
        let spacing = WorkflowCanvasMetrics.gridSpacing * camera.zoom
        guard spacing > Space.sm else { return }

        let dot = AppColors.textTertiary.opacity(0.3)
    ...
    private func drawMarquee(in context: inout GraphicsContext) {
        ...
        context.stroke(
            path,
            with: .color(AppColors.brand.opacity(0.7)),
...
private struct ScrollPinchCapture: NSViewRepresentable {
    let onScroll: (
        _ delta: CGSize, _ location: CGPoint, _ isPrecise: Bool, _ isCommandDown: Bool
    ) -> Void

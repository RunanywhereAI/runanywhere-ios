...
//  RunAnywhereAI
//
//  Five-field cron, parsed and evaluated in the app.
//
...
///
/// Ported from `core/src/agent/cron.cpp`, which is leaving the SDK: workflows
/// belong to the app now, and this is the first piece to come across because it
/// is pure arithmetic with no dependency on the workflow types.
///
...
struct CronExpression: Equatable {
    ...
    func nextDate(after date: Date, calendar: Calendar) -> Date? {
        ...
        let start = calendar.date(
            ...
            to: calendar.date(from: calendar.dateComponents([.year, .month, .day, .hour, .minute], from: date)) ?? date
        )
